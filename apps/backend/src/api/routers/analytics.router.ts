...
router.get('/dashboard', authenticate, async (_req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      Lead.countDocuments({ opportunityLevel: 'high', status: { $nin: ['archived', 'won', 'lost'] } }),
      Lead.countDocuments({ opportunityLevel: 'medium', status: { $nin: ['archived', 'won', 'lost'] } }),
      Lead.countDocuments({ opportunityLevel: 'low', status: { $nin: ['archived', 'won', 'lost'] } }),
      ...
      getQueueStats(),
      Lead.aggregate([
        { $match: { opportunityScore: { $gt: 0 } } },
        { $bucket: { groupBy: '$opportunityScore', boundaries: [0, 15, 30, 45, 65, 80, 101], default: 'other',
            output: { count: { $sum: 1 } } } },
      ]),
    ]);
    ...
    const daily = await Lead.aggregate([
      { $match: { createdAt: { $gte: since } } },
      {
        $group: {
          ...
          total: { $sum: 1 },
          high: { $sum: { $cond: [{ $eq: ['$opportunityLevel', 'high'] }, 1, 0] } },
          medium: { $sum: { $cond: [{ $eq: ['$opportunityLevel', 'medium'] }, 1, 0] } },
          low: { $sum: { $cond: [{ $eq: ['$opportunityLevel', 'low'] }, 1, 0] } },
        },
      },
      { $sort: { _id: 1 } },
    ]);
